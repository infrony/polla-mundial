...
function resolveStatus(game) {
  ...
  // Use time_elapsed if the API provides it
  ...
  // No time_elapsed: infer from local_date (format: "MM/DD/YYYY HH:mm")
  // If the scheduled time has passed and game isn't finished → treat as in progress
  if (game.local_date) {
    try {
      const [datePart, timePart] = game.local_date.split(' ');
      const [month, day, year] = datePart.split('/');
      // Parse as UTC approximation (local_date is ET/local, close enough for "started?" check)
      const kickoff = new Date(`${year}-${month}-${day}T${timePart}:00`);
      const minsElapsed = (Date.now() - kickoff.getTime()) / 60000;
      if (minsElapsed >= 0 && minsElapsed < 120) return minsElapsed <= 50 ? '1H' : '2H';
    } catch {}
  }
