...
export async function syncKnockoutBracket() {
  ...
  for (const g of koGames) {
    ...
    // 1. By matchday (most reliable when the API provides it)
    // 2. By team-name pair (if teams already set)
    // 3. First empty slot as fallback
    let slot = null;
    if (g.matchNumber) {
      slot = slots.find(s => s.match_number === g.matchNumber);
    }
    if (!slot && g.homeEs && g.awayEs) {
      slot = slots.find(s =>
    ...
    // Guard: skip results for games whose local date is still in the future.
    // The API occasionally marks upcoming games as finished prematurely, which
    // would write a wrong result into the DB before the match is played.
    const todayUtc = new Date().toISOString().split('T')[0]; // "YYYY-MM-DD"
    const gameDate = g.localDate ? g.localDate.split(' ')[0] : '';
    if (gameDate && gameDate > todayUtc) {
      updated.push(`skipped-future:${round}#${slot.match_number}(${gameDate})`);
      continue;
    }
