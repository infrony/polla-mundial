...
export async function syncKnockoutBracket() {
  ...
  for (const g of koGames) {
    ...
    // Record result when finished and there's a clear 90-min winner
    if (!g.isFinished || g.homeScore === null || g.awayScore === null) continue;
    if (g.homeScore === g.awayScore) continue; // tied at 90 min — wait for ET/PEN

    const winner = g.homeScore > g.awayScore ? g.homeEs : g.awayEs;
    const loser  = g.homeScore > g.awayScore ? g.awayEs : g.homeEs;

    await query(`
      INSERT INTO knockout_results (match_id, winner, entered_at)
      VALUES ($1, $2, NOW())
      ON CONFLICT (match_id) DO UPDATE SET winner = EXCLUDED.winner, entered_at = NOW()
    `, [slot.id, winner]);
    updated.push(`result:${g.round}#${slot.match_number}=${winner}`);
